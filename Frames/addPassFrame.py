...
class AddPassFrame(tk.Frame):
	def __init__(self, parent, controller):
		...
		self.pwGenBtn = tk.Button(self.addPassFrame, text = "Generate Password", command=self.generatePass, bg = self.primaryColor, fg = self.secTextColor, font = self.labelFont)
		self.pwGenBtn.place(relx=0.175, rely=0.7, relwidth=0.3, relheight=0.07)	
		self.saveBtn = tk.Button(self.addPassFrame, text = "Save Password", command=self.savePass, bg=self.successColor, fg=self.priTextColor, font = self.labelFont)
